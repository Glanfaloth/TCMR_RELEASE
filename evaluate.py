...
from lib.utils.eval_utils import compute_accel, compute_error_accel, batch_compute_similarity_transform_torch, \
    compute_error_verts, compute_errors, plot_accel
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for seq_name in pbar:
            ...
            chunk_idxes = split_into_chunks(vid_names, seqlen=seqlen, stride=stride, is_train=False,
                                            match_vibe=False)  # match vibe eval number of poses
            if chunk_idxes == []:
                continue
            ...
            target_theta = np.concatenate([dummy_cam, dataset_data[seq_name]['pose'], dataset_data[seq_name]['shape']],
