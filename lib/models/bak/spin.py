...
class Regressor(nn.Module):
    def __init__(self, smpl_mean_params=SMPL_MEAN_PARAMS):
        ...
        # npose = 24 * 6
        npose = 49 * 3
        self.fc1 = nn.Linear(2195+147, 1024)
        self.drop1 = nn.Dropout()
        ...
        mean_params = np.zeros(147,dtype=np.float)
        init_pose = torch.from_numpy(mean_params).unsqueeze(0)
        # init_shape = torch.from_numpy(mean_params['shape'][:].astype('float32')).unsqueeze(0)
        # init_cam = torch.from_numpy(mean_params['cam']).unsqueeze(0)
        self.register_buffer('init_pose', init_pose)
        # self.register_buffer('init_shape', init_shape)
    ...
    def forward(self, x, o,init_pose=None, init_shape=None, init_cam=None, n_iter=3, is_train=False, J_regressor=None):
        batch_size = x.shape[0]
        ...
        # if init_cam is None:
        #     init_cam = self.init_cam.expand(batch_size, -1)
        if batch_size==o.shape[0]:
            ego_feature = o[:,0,:,:].view(-1,147)
        else:
            ego_feature = o.view(-1, 147)
        pred_pose = init_pose.float()

        # ego_feature = self.att(ego_feature)


        # pred_shape = init_shape
        # pred_cam = init_cam
        for i in range(n_iter):
            xc = torch.cat([x, pred_pose,ego_feature], 1)
            xc = self.fc1(xc)
            xc = self.drop1(xc)
            xc = self.fc15(xc)
            xc = self.drop15(xc)
        ...
        # pred_vertices = pred_output.vertices
        # pred_joints = pred_output.joints
        pred_joints = pred_pose.view(-1,49,3)
